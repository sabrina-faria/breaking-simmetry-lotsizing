...
def constraint_simetria_do_carryover(mdl: Model, data: dataCS) -> Model:
    for t in range(1,data.nperiodos):
        for j in range(1, data.r):
                mdl.add_constraint(mdl.z[0,j,t-1] == mdl.y[0,j,t])
                for i in range(1,data.nitems):
                    mdl.add_constraint(mdl.z[i,j,t-1] >= mdl.y[i,j,t] - mdl.sum(mdl.y[u,j,t] for u in range(i)))
    return mdl
...
def build_model(data: dataCS, capacity: float) -> Model:    
    ...
    mdl = constraint_simetria_do_carryover(mdl, data)
