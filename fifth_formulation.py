...
def constraint_condicional(mdl: Model, data: dataCS) -> Model:
    for t in range(data.nperiodos):
        for j in range(1,data.r):
            if t ==0:
                mdl.add_constraint(mdl.sum(2 ** (data.nitems - i) * mdl.y[i, j-1, t] for i in range(data.nitems))>= mdl.sum(2 ** (data.nitems - i) * mdl.y[i, j, t] for i in range(data.nitems)))
            else:
                ...
                mdl.add_constraint(mdl.sum(2 ** (data.nitems - i) * mdl.y[i, j - 1, t] for i in range(data.nitems))
                    >= mdl.sum(2 ** (data.nitems - i) * mdl.y[i, j, t] for i in range(data.nitems)) - 1000000*(1 - mdl.w[t]))
    return mdl
